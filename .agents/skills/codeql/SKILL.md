...
metadata:
  local_modifications: "Adds repository-specific approval rules for scans, dependency installation, and cleanup."
---
...
- Keep diagnosis read-only until the user has explicitly approved a scan that creates artifacts.
- Do not install dependencies, delete databases or build artifacts, or modify the target environment without explicit approval.
- Resolve and validate every cleanup target before using commands from the troubleshooting references.
