...
- Diagnose with read-only Azure, Kubernetes, and MCP operations by default.
- Do not change cloud resources, reset credentials, restart workloads, run privileged debug pods, or perform upgrades without explicit user approval and confirmed target scope.
- Treat every remediation command in the references as a proposal until its impact and rollback path are understood.
